...
RECIPIENT    = os.environ["RECIPIENTS_JSON"]  # single recipient (simple)
...
def parse_age_to_minutes(s: str) -> int:
    """Tiny, robust Age parser; minutes smaller => newer."""
...
def extract_rows_with_links(md_text: str):
    """Markdown -> HTML -> parse tables with hrefs kept."""
    try:
...
def load_template() -> str:
    ...
    <!doctype html><html><body style="font-family:Arial">
      ...
      <p style="color:#777;font-size:12px;margin-top:12px">Source: SimplifyJobs/New-Grad-Positions (personal reminder only).</p>
    </body></html>
    """
...
def main():
    ...
    # compute Age minutes + IDs; newest first
    for r in rows:
    ...
    html = render_with_template(
        "Latest 50 jobs (manual or 9AM run)",
        render_rows_html(latest_50),
        dt.datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
    )
    send_email("[Jobs Digest] Latest 50 jobs", html)

    # Save their IDs into state (union)
    state = load_state()
    new_ids = state.union({r["ID"] for r in latest_50})
    save_state(new_ids)
    print(f"Seed run: emailed latest 50; state now has {len(new_ids)} IDs.")
